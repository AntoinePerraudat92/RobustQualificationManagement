...
@pytest.mark.parametrize("cls", [RecourseProblem, DualRecourseProblem])
def test_simple_problem_with_two_products_and_one_factory(cls):
    nmb_products = 2
    nmb_factories = 1
...
@pytest.mark.parametrize("cls", [RecourseProblem, DualRecourseProblem])
def test_simple_problem_with_two_products_dedicated_factories(cls):
    nmb_products = 2
    nmb_factories = 2
    ...
    factory_capacities = np.array([10, 25], dtype=np.float64)
    dataset: Dataset = Dataset(nmb_products, nmb_factories, qualification_matrix, qualification_costs, lost_sales_cost,
                               factory_capacities)
    demand_scenario: DemandScenario = DemandScenario(product_demands=np.array([11.1, 26.4], dtype=np.float64))

    problem = cls(dataset)

    feasible = problem.solve(qualification_matrix=qualification_matrix, demand_scenario=demand_scenario)
    assert feasible is True
    assert 2.5 == pytest.approx(problem.get_lost_sales())
